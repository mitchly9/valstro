# Valstro Star Wars API

## Running Instructions

- git clone https://github.com/mitchly9/valstro.git
- cd valstro
- npm install
- docker run -p 3000:3000 aaronbate/socketio-backend
- npx tsc
- node build/index.js

## Example Output

Type "Exit" at any time to leave.
A long time ago in a galaxy far, far away…

What character would you like to search for? dar
(1/3) Darth Vader - [A New Hope, The Empire Strikes Back, Return of the Jedi, Revenge of the Sith]
(2/3) Darth Maul - [The Phantom Menace]
(3/3) Darth Sidious - [The Phantom Menace, Attack of the Clones, Revenge of the Sith]

What character would you like to search for? Exit
May the Force be with you…
